...
print(asd_k_cluster.columns.values)
...
distance = euclideanDistance(asd_k_cluster["age"], asd_k_cluster["result"], 3)
...
asd_k_cluster.replace('-', np.nan, inplace = True)
asd_k_cluster = asd_k_cluster.dropna()


# In[1495]:


iX = asd_k_cluster.drop(columns = ["result"])
iX.shape
...
lm = smf.ols('age ~ result',asd_k_cluster).fit()
lm.summary()
